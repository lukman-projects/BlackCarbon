...
const EditAvatarModal = () => {
    ...
    const uploadImage = (e) => {
        dispatch(setFormName(modalNames.EDITUSERAVATAR));
        e.preventDefault();
        if(file){
            changeProfile({
                info: {avatar: file},
    ...
    return (
        <div className={`${checkModal(modalState.name, modalNames.EDITUSERAVATAR, modalState.isActive) ? "active": ""} checkKey w-full h-full fixed left-0 top-0  flex justify-center items-center`}>
            <div className="checkKey-modal  relative">
                <div className={`form-loading ${checkModal(formState.name, formNames.EDITUSERAVATAR, formState.isLoading) ? "active": ""} absolute left-0 top-0 w-full  h-full flex justify-center items-center`}>
                ...
                <form onSubmit={uploadImage} className="checkKey__form grid grid-cols-1 justify-center">
                    ...
                    {formState.errors &&  formState.errors.avatar && formState.errors.avatar.map((el, i) => {
